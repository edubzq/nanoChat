...
public class NCShell {
	...
	private String[] readChatCommandFromStdIn(NCConnector ncclient) {
		...
		while (true) {
			...
			case NCCommands.COM_WHISP:
				vargs.add(st.nextToken());
				StringBuffer whisp = new StringBuffer();
				while (st.hasMoreTokens())
					whisp.append(st.nextToken()+" ");
				vargs.add(whisp.toString());
				break;
	...
	private boolean validateCommandArguments(String[] args) {
		...
		//kick requiere el parametro <user>
		case NCCommands.COM_KICK:
			if (args.length == 0 || args.length > 1) {
				System.out.println("Correct use: kick <user>");
				return false;
			}
			break;
		//promote requiere el parametro <user>
		case NCCommands.COM_PROMOTE:
			if (args.length == 0 || args.length > 1) {
				System.out.println("Correct use: promote <user>");
				return false;
			}
			break;
		...
		case NCCommands.COM_NICK:
			...
			break;
		//create requiere el parametro <room>
		case NCCommands.COM_CREATE_ROOM:
			if (args.length == 0 || args.length > 1) {
		...
		case NCCommands.COM_SEND:
			...
			break;
		// rename requiere el parámetro <new_name>
		case NCCommands.COM_RENAME:
			if(args.length == 0) {
				System.out.println("Correct use: rename <new_name>");
				return false;
			}
			break;
		// whisp requiere los parámetros <user> y <message>
		case NCCommands.COM_WHISP:
			if(args.length < 2) {
				System.out.println("Correct use: rename <new_name>");
				return false;
